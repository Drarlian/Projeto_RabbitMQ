...
def process_excel(file_content):
    ...
    ws = wb.active
    cell_value = ws['A1'].value  # Exemplo de leitura de uma célula
    # LOGICA DO CÓDIGO EXCEL.
    return cell_value  # Retorna um exemplo de valor para demonstração


def callback(ch, method, properties, body):
    result = process_excel(body)
...
"""
channel.queue_declare(queue='excel_processing')

"""
Consumo de mensagens da fila: Define a função de callback para processar mensagens recebidas da fila 'excel_processing'.
"""
channel.basic_consume(queue='excel_processing', on_message_callback=callback)
